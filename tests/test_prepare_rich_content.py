...
class TestPrepareRichContent(TestPlatformClient):
    ...
    def test_prepare_rich_content(self, layerclient, monkeypatch):
        ...
        def verify_request_args(method, url, headers, data):
            assert method == 'POST'
            assert url == (
                'https://api.layer.com/apps/TEST_APP_UUID/conversations/f3cc7b32-3c92-11e4-baad-164230d1df67/content'
            )
            assert headers == {
        ...
        content = b'some image encoded data'
        rich_content = layerclient.prepare_rich_content(conversation, 'image/jpeg', 23)
